...
export async function updateDeliveryStatus(statusData) {
    ...
    const [oldDeliveryTracking] = await query(
        `
  ...
  `,
    ...
    if (oldDeliveryTracking) {
        return await updateEntity(oldDeliveryTracking.id, { status, driver_id }, 'delivery_tracking');
    } else {
...
export async function calculateETA(orderId, driverId, latitude, longitude) {
    // Get current driver location
  ...
  `,
    ...
    if (!driverLocation.length && !latitude && !longitude) {
  ...
  `,
    ...
    const distance = await calculateDistance(
        driverLocation.length ? parseFloat(driverLocation[0].latitude) : parseFloat(latitude),
        driverLocation.length ? parseFloat(driverLocation[0].longitude) : parseFloat(longitude),
        parseFloat(deliveryLocation[0].latitude),
        parseFloat(deliveryLocation[0].longitude)
    );

    console.log('distance', distance);
    console.log('avgSpeedKmh', avgSpeedKmh);
