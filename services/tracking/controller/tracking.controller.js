...
export async function updateDriverLocation(locationData) {
  const { driver_id, order_id, latitude, longitude, speed, heading, accuracy } =
    locationData;
...
export async function getNearbyDeliveries(latitude, longitude, radiusKm = 5) {
  ...
  const deliveries = await findNearbyDeliveries(
    parseFloat(latitude),
...
export async function calculateETA(orderId, driverId, latitude, longitude) {
  ...
  const distance = calculateDistance(
    driverLocation.length ? driverLocation[0].latitude : latitude,
    driverLocation.length ? driverLocation[0].longitude : longitude,
