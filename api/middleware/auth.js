...
export async function authenticateToken(req, res, next) {
    try {
        ...
        if (!token) {
            return return401(res, 'Token de acesso requerido');
        }

        const authService = serviceRegistry.getService('authentication');

        if (!authService) {
            return return500({ message: 'Authentication service not available' }, req, res);
        }
...
export function requireRole(role) {
    return (req, res, next) => {
        if (!req.user) {
            return return401(res, 'Usuário não autenticado');
        }
